my_puzzle = '''bqyqwn (68) -> wscqe, cwxspl, syogw, xnxudsh
ddswb (34)
hnkvw (320)
...
def prepare_data(input_data):
    ...
    for line in input_data.split('\n'):
        line_data = {'name': '', 'weight': 0, 'dependencies': []}
        for i, element in enumerate(line.split('->')):
            if i == 0:
                name, weight = element.split()
                weight = int(weight.replace('(', '').replace(')', ''))
                line_data['name'], line_data['weight'] = name, weight
            else:
                line_data['dependencies'] = element.replace(' ', '').split(',')
        data.append(line_data)
    return data


def find_root(data):
    programs = [program['name'] for program in data]
    for program in data:
        for dependency in program['dependencies']:
            programs.remove(dependency)
    return programs[0]


def recursive_weight(program, data_dict):
    weight = program['weight']
    for dependency in program['dependencies']:
        weight += recursive_weight(data_dict[dependency], data_dict)
    return weight
...
def find_weight(data):
    root = find_root(data)
    data_dict = {
        program['name']: {'weight': program['weight'], 'dependencies': program['dependencies']} for program in data
    }
    sub_programs_weights = {}
    for dependency in data_dict[root]['dependencies']:
        sub_programs_weights[recursive_weight(data_dict[dependency], data_dict)] = data_dict[dependency]['weight']
    print(sub_programs_weights)
    return sub_programs_weights[max(sub_programs_weights)] - (max(sub_programs_weights) - min(sub_programs_weights))


assert find_weight(prepare_data(first_input)) == 60

print(find_weight(prepare_data(my_puzzle)))
